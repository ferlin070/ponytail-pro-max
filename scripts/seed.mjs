#!/usr/bin/env node
/**
 * seed — starter data + DESIGN.md for the current app.
 *
 * Writes DESIGN.md (from designs/, picked by keyword) if missing, and
 * src/seedData.ts + a test using src/lib/seed.ts makeSeed. Never overwrites
 * existing files — you fill the factory with your real fields.
 *
...
const DESIGNS = ['fintech.md', 'dashboard.md', 'ecommerce.md', 'mobile-first.md', 'minimal.md'];
...
// The record shape here MUST match the type guard in src/storage.ts.
...
console.log('      2. Mirror the fields into the type guard in src/storage.ts');
console.log('      3. Seed on first run: store.save(seedItems()) when empty');