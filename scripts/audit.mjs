...
function readAll(dir) {
  ...
  function walk(d) {
    for (const e of readdirSync(d, { withFileTypes: true })) {
      if (['node_modules', 'dist', '.git', 'designs', 'react-tailwind'].includes(e.name)) continue;
...
const modular = ['types.ts', 'schema.ts', 'storage.ts'].filter((f) => existsSync(join(ROOT, 'src', f))).length >= 2;
t.push(['TypeScript strict', /"strict"\s*:\s*true/.test(tsconfig)]);
t.push(['Modular files (types/schema/storage)', modular]);
t.push(['Unit tests present', /describe\(|it\(|test\(/.test(src)]);
t.push(['escapeHtml before innerHTML', /escapeHtml/.test(src)]);
